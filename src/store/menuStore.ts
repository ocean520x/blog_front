import type { MenuStore, MyMenu } from '@/interfaces/interface'
...
export default defineStore('menu', (): MenuStore => {
  ...
  function getMenuByRoutes() {
    menus.value = routes
      ...
      .map((route) => {
        const menu = route.meta?.menu as MyMenu
  ...
  function addHistoryMenus(route: RouteLocationNormalizedLoadedGeneric) {
    ...
    if (!hasSame)
      historyMenus.value.unshift(historyMenu)
    if (historyMenus.value.length > 10)
      historyMenus.value.pop()
